...
import { removeCollectionCommand } from './remove';
...
export async function listCollections(): Promise<void> {
  while (true) {
    ...
    // Show interactive table with action shortcuts
    const action = await showInteractiveTable(tableData, {
    ...
    switch (action) {
      ...
      case 'edit':
        ...
        break;
      case 'remove':
        await removeCollectionCommand();
        break;
    }
    // Loop continues — table refreshes with updated data
  }
}
